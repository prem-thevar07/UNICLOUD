...
export class DropboxAdapter extends ProviderAdapter {
  ...
  normalizeMetadata(file) {
    ...
    return {
      ...
      thumbnailUrl: file.thumbnailLink || `/api/dropbox/open/${this.accountId}?path=${encodeURIComponent(file.id)}`,
      previewUrl: `/api/dropbox/open/${this.accountId}?path=${encodeURIComponent(file.id)}`,
